...
function slideXml(title: string, body: string, footer: string | null): string {
  ...
  const shapes = [
    ...
    textBox(3, 'Body', { x: margin, y: Math.round(1.9 * EMU_PER_INCH), cx: width, cy: Math.round(4.6 * EMU_PER_INCH) }, paragraphs(body, 1600, '374151')),
  ];
  if (footer) {
    shapes.push(
