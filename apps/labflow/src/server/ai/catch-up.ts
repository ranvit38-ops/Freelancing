...
export async function buildCatchUpContext(s: SessionContext): Promise<string> {
  ...
  if (shared.length > 0) {
    lines.push('', 'SHARED FILES (names only; contents not supplied)');
...
export async function catchUp(s: SessionContext, fetchImpl?: ModelTransport): Promise<string> {
  const context = await buildCatchUpContext(s);
  const { text } = await callModel(
    { system: CATCH_UP_RULES, prompt: `RECORDS\n\n${context}\n\nWrite the briefing now.`, maxTokens: 4000, effort: 'low' },
    fetchImpl,
  );
  return text;
}
